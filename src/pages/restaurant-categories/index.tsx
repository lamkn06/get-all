...
const RestaurantCategories: React.FC<Props> = () => {
  ...
  const fetchData = async () => {
    ...
    const { data } = await restaurantCategoryService.get(params);
    ...
    showConfirm({
      ...
      onOk() {
        ...
        return new Promise(async (resolve, reject) => {
          try {
            await restaurantCategoryService.delete(category.id);
  ...
  const handleSubmit = async () => {
    try {
      ...
      let id = '';
      if (currentCate) {
        await restaurantCategoryService.update({
          ...currentCate,
          ...payload,
        });
        id = currentCate.id;
      } else {
        const res = await restaurantCategoryService.create(payload);
        id = res.data.id;
      }

      if (formValues.image && formValues.image[0].originFileObj) {
        data.append('file', formValues.image[0].originFileObj);
        await restaurantCategoryService.upload(id, data);
      }
